...
Users and admins can modify user metadata, users through the xref:account.adoc[Account Console], and administrators through the Admin Console. The `Edit Mode` configuration on the LDAP configuration page defines the user's LDAP update privileges.
...
Enable Kerberos/SPNEGO authentication in the realm with user data provisioned from LDAP. For more information, see the xref:authentication/kerberos.adoc[Kerberos section].
