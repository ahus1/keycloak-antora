...
* Fine grain permissions are only available within xref:admin-console-permissions/per-realm.adoc[dedicated admin consoles] and admins defined within those realms.  You cannot define cross-realm fine grain permissions.
...
.Client management
image:fine-grain-client.png[Fine grain client]

You should see a tab menu item called `Permissions`.  Click on that tab.

.Client permissions tab
image:fine-grain-client-permissions-tab-off.png[Fine grain client permissions tab]
...
.Client permissions tab
image:fine-grain-client-permissions-tab-on.png[Fine grain permission tab]
...
image:fine-grain-client-manage-permissions.png[Fine grain client manage permission ]
...
.User policy
image:fine-grain-client-user-policy.png[Fine grain client user policy]
...
.Assign user policy
image:fine-grain-client-assign-user-policy.png[Fine grain client assign user policy]
...
image:fine-grain-assign-query-clients.png[Fine grain assign query clients]
...
Next, we log out of the master realm and re-login to the xref:admin-console-permissions/per-realm.adoc[dedicated admin console] for the `test` realm
using the `sales-admin` as a username.  This is located under `{kc_admins_path}/test/console`.

.Sales admin login
image:fine-grain-sales-admin-login.png[Fine grain sales admin login]
...
image:fine-grain-sales-application-roles.png[Fine grain sales application roles]
...
image:fine-grain-view-leads-role-tab.png[Fine grain view leads role]
...
image:fine-grain-view-leads-permissions.png[Fine grain view leads permissions]
...
image:fine-grain-map-roles-permission.png[Fine grain map roles permission]
...
.Users permissions
image:fine-grain-users-permissions.png[Fine grain user permissions]
...
.Add view-users
image:fine-grain-add-view-users.png[Fine grain add view users]
...
.Add viewleads
image:fine-grain-add-view-leads.png[Fine grain add view leads]
...
image:fine-grain-client-permissions-tab-on.png[Fine grain client permissions]
