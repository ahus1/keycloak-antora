...
{project_name} brokers identity providers based on the OpenID Connect protocol. These identity providers (IDPs) must support the xref:sso-protocols.adoc#con-oidc-auth-flows[Authorization Code Flow] defined in the specification to authenticate users and authorize access.
...
+
. Enter your initial configuration options. See xref:identity-broker/configuration.adoc[General IDP Configuration] for more information about configuration options.
+
...
|Client Secret
|Client secret from an external xref:vault.adoc[vault]. This secret is necessary if you are using the Authorization Code Flow.
...
If your identity provider's private key is compromised, update your keys and clear the keys cache. See xref:admin-console.adoc#_clear-cache[Clearing the cache] for more details.
endif::[]
