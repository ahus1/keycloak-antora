...
Ensure you have a complex password policy to force users to choose complex passwords. See the xref:authentication/password-policies.adoc[Password Policies] chapter for more information. Prevent password guessing by setting up the {project_name} server to use one-time-passwords.
