...
The IDP initiated login implementation prefers _POST_ over _REDIRECT_ binding (check xref:identity-broker/saml.adoc[saml bindings] for more information).
...
When using xref:identity-broker-all.adoc[identity brokering], it is possible to set up an IDP Initiated Login for a client from an
...
just use xref:identity-broker/saml.adoc#_identity_broker_saml_sp_descriptor[SP Descriptor] available from the settings of the identity provider in
