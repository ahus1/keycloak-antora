...
See the xref:assembly-managing-clients.adoc#_service_accounts[Service Accounts] chapter for more information.
...
To configure clients to receive logout requests through the front-channel, look at the xref:assembly-managing-clients.adoc#_front-channel-logout[Front-Channel Logout] client setting. When using this method, consider the following:
...
using the xref:assembly-managing-clients.adoc#_back-channel-logout-url[Back-Channel Logout URL]. If not defined, the server is going to fall back to using the xref:assembly-managing-clients.adoc#_admin-url[Admin URL].
